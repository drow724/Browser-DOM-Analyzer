import { composedParent, isPrivateTree, sanitizeText } from '../privacy/sanitize.js';
...
export function roleOf(el) {
    ...
    if (explicit)
        return { role: explicit.slice(0, 64), roleSource: 'explicit' };
    ...
    if (tag === 'input') {
        const type = el.type;
        role = { checkbox: 'checkbox', radio: 'radio', range: 'slider', number: 'spinbutton', search: 'searchbox', button: 'button', submit: 'button', reset: 'button' }[type];
        if (!role && !['hidden', 'password', 'color', 'file', 'image'].includes(type))
            role = 'textbox';
    }
    return role ? { role, roleSource: 'implicit' } : {};
}
export function ownsText(el) {
...
export function readSafeText(el, options, max, allowHidden = false) {
    if (isPrivateTree(el, options))
        return '';
    for (let ancestor = el; ancestor; ancestor = composedParent(ancestor)) {
        if (blockedTags.has(ancestor.localName))
            return '';
    }
    const stack = [el];
    let text = '', visits = 0;
    while (stack.length && text.length < max * 4 && visits++ < 2048) {
        ...
        if (node.nodeType !== 1)
            continue;
        const current = node;
        if (blockedTags.has(current.localName) || ['input', 'textarea', 'select', 'option'].includes(current.localName) || isPrivateTree(current, options))
            continue;
        if (current !== el && /^(button|a|summary)$/.test(current.localName))
            continue;
        if (!allowHidden) {
            const style = current.ownerDocument.defaultView.getComputedStyle(current);
            if (style.display === 'none' || Number(style.opacity) === 0 || current.getAttribute('aria-hidden') === 'true')
                continue;
...
export function extractSemantic(el, options, max, suppressText = false) {
    const result = roleOf(el);
    ...
    const clean = (text, kind) => sanitizeText(text, kind, max, options);
    ...
    const labelled = references.map(id => root.getElementById?.(id)).filter((x) => Boolean(x))
        .map(target => readSafeText(target, options, max, true)).join(' ');
    const labels = 'labels' in el ? Array.from(el.labels || []).slice(0, 16)
