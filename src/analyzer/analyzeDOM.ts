...
import { isPrivateSelf, sanitizeText, sanitizeURL, type PrivacyOptions } from '../privacy/sanitize.js';
...
function analyzeDOMInternal(document: Document, options: AnalyzeOptions, captureBindings: boolean): InternalAnalysisResult {
  ...
  if (!Number.isInteger(maxText) || maxText < 0 || maxText > 10_000) throw new RangeError('maxTextLength must be an integer in [0, 10000]');
  if (!Number.isInteger(maxElements) || maxElements < 1) throw new RangeError('maxElements must be a positive integer');
  const privacy = options.privacy ?? {};
  // Fail early for invalid caller selectors. No partial traversal silently accepted.
  for (const selector of privacy.excludeSelectors ?? []) document.documentElement.matches(selector);
  ...
  const states = new WeakMap<Element, WalkState>();
  const filter = options.filter ?? defaultFilter;
  for (const { element: el, parent, scopeHost } of traverseDOM(document.documentElement)) {
    if (model.stats.scannedElements >= maxElements) { model.coverage.truncated = true; break; }
    ...
    const state: WalkState = { parentId: inherited?.parentId, blocked: inherited?.blocked ?? false,
      textOwned: inherited?.textOwned ?? false, inert: inherited?.inert ?? false,
      ariaHidden: inherited?.ariaHidden ?? false, scopeId };
    states.set(el, state);
    if (state.blocked && !isRoot) continue;
    ...
    if ((markerExcluded || blockedTags.has(el.localName)) && !isRoot) { state.blocked = true; continue; }
    const style = win.getComputedStyle(el);
    const rect = readRect(el);
    const visible = visibility(style, rect);
    if (visible.blockSubtree && !isRoot) { state.blocked = true; continue; }
    ...
    if (parent?.localName === 'details' && !parent.hasAttribute('open') &&
      el !== Array.from(parent.children).find(child => child.localName === 'summary')) { state.blocked = true; continue; }
    ...
    if (!isRoot && !visible.visible && !meaningfulBoxless && !el.shadowRoot && el.localName !== 'slot') continue;
    ...
    if (state.ariaHidden) semantic.ariaHidden = true;
    if (el.localName === 'a' && el.hasAttribute('href')) {
      const href = sanitizeURL(el.getAttribute('href')!, document.baseURI, privacy.urlMode);
      if (href) interaction.href = href;
    }
    const decision = filter({ element: el, style, visible: visible.visible, semantic, interaction, hasOwnText: Boolean(semantic.text) });
    if (decision === 'exclude-subtree' && !isRoot) { state.blocked = true; continue; }
    if (decision === 'collapse' && !isRoot && !el.shadowRoot && el.localName !== 'slot') continue;
    const id = `n${index}`;
    const node: SiteNode = { id, identity: { tag: el.localName }, semantic, interaction,
    ...
    if (privacy.includeIdentity && !isPrivateSelf(el, privacy)) {
      ...
      if (domId) node.identity.id = domId;
      const classes = Array.from(el.classList).sort().slice(0, 16).map(c => sanitizeText(c, 'identity', 80, privacy)).filter((c): c is string => Boolean(c));
      if (classes.length) node.identity.classes = classes;
    }
    const z = Number(style.zIndex);
    if (style.zIndex !== 'auto' && Number.isFinite(z)) node.layout.zIndex = z;
    if (options.styles !== false) node.design = { styleRef: registry.intern(normalizeStyle(style)) };
    if (el.shadowRoot) {
